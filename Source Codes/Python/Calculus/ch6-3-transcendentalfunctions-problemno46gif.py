...
# R is a region in the xy plane bounded by y = e^(-x^2), y=0, x=0, and x=1
# Create the animation of A solid of revolution that is formed by rotating R around the y axis. 
import gif
import numpy as np
...
def plot_volume(angle):
    ...
    x = np.linspace(1/np.exp(1),1, 60)
    v = np.linspace(0, 2*angle, 60)

    U, V = np.meshgrid(x, v)
    Y1 = np.sqrt(-np.log(U))*np.cos(V)
    Z1 = np.sqrt(-np.log(U))*np.sin(V)
    X = U
