...
plot(xs1, y, color=:red, 
	xlims=(0,1.2), ylims=(-π/2,2π),framestyle=:zerolines,
	linestyle=:solid,  linecolor=:red2,
	legend=:topleft, label=L"sech^{-1} \ x", 
	bottom_margin=3mm,
	size=(800, 460), tickfontsize=10)

# First Derivative plot
plot!(xs1, ys1, label=L"D_{x} sech^{-1} \ x ",
	color=:blue2)

# Second Derivative plot
plot!(xs1, ys2, label=L"D^{2}_{x} sech^{-1} \ x ", color=:green3)
