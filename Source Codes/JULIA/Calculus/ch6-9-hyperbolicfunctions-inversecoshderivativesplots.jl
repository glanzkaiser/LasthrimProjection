...
plot(xs1, y, color=:red, ytick=pitick(a, b, 8; mode=:latex), 
	xlims=(-3,3), ylims=(-π/2,π/2),framestyle=:zerolines,
	linestyle=:solid,  linecolor=:red2,
	legend=:topleft, label=L"\cosh^{-1} \ x", 
	bottom_margin=3mm,
	size=(800, 460), tickfontsize=10)

# First Derivative plot
plot!(xs1, ys1, label=L"D_{x} \cosh^{-1} \ x ",
	color=:blue2)

# Second Derivative plot
plot!(xs1, ys2, label=L"D^{2}_{x} \cosh^{-1} \ x ", color=:green3)
