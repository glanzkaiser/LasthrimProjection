using SymPy, CalculusWithJulia, LaTeXStrings,Plots,PrettyTables
@syms x,y, u()

F(y,x) = y # The differential equation is y' = y
x0, y0 = 0, 1 # The initial condition is y(0)=1

# For calculation that will be shown with Pretty Table below
n = 21
h = 0.05
xs = zeros(n+1)
ys = zeros(n+1)
xs[1] = x0   # index is off by 1
ys[1] = y0
for i in 1:n
  xs[i + 1] = xs[i] + h
  ys[i + 1] = ys[i] + h * F(ys[i], xs[i])
end

println("Euler's Method:")
println("The differential equation: y' = y")
println("Initial condition: y(0) = 1")
println("interval [0,1]")
println("")
println("Exact solution:")

# To calculate the analytic solution
D = Differential(x)
dsolve(D(u)(x) - F(u(x), x))
out = dsolve(D(u)(x) - F(u(x),x), u(x), ics=Dict(u(x0) => y0))
sympy.pretty_print(rhs(out))

println("")
println(pretty_table(String, 
 	 vcat([[(j*h)-h ys[j] exp((j*h)-h)] for j in 1:1:n]...); 
  	header=["x_{n}", "y_{n}", "e^{x_{n}}"]))

# To plot
p = plot(legend=:bottomright)
vectorfieldplot!((x,y) -> [1, F(x,y)], xlims=(0, 1.1), ylims=(0, 4))
plot!(rhs(out), linecolor=:green, linewidth=3, label=L"e^{x_{n}}")
plot!(xs, ys, linecolor=:orange, linewidth=3, label=L"y_{n}")
scatter!(xs,ys, label="")