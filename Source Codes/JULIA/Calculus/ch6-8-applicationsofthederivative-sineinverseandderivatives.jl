...
plot(ys, xs, color=:red, ytick=pitick(a, b, 2; mode=:latex), 
	...
	legend=:topleft, label=L"\sin^{-1} \ x", 
	bottom_margin=3mm,
...
plot!(xs1, ys1, label=L"D_{x} \sin^{-1} \ x = \frac{1}{\sqrt{1 - x^{2}}}", framestyle=:zerolines)

plot!(xs2, ys2, label=L"D^{2}_{x} \sin^{-1} \ x = \frac{x}{\sqrt{(1-x^{2})^{3}}}", framestyle=:zerolines)
