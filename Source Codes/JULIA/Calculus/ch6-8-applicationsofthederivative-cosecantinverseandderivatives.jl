...
plot(xs, y, color=:red, ytick=pitick(-b/2, b/2, 2; mode=:latex), 
	xlims=(-3,3), ylims=(-π,π),framestyle=:zerolines,
	linestyle=:solid,  linecolor=:red2,
	legend=:topleft, label=L"\csc^{-1} \ x", 
	bottom_margin=3mm,
	size=(800, 460), tickfontsize=10)# the inverse function
plot!(xs1, y, color=:red, label="")

# First Derivative plot
plot!(xs1, ys1, label=L"D_{x} \csc^{-1} \ x = - \frac{1}{|x| \sqrt{x^{2}-1}}",
	color=:blue2)
plot!(xs, ys1, label="",color=:blue2)

# Second Derivative plot
plot!(xs1, ys2, label=L"D^{2}_{x} \csc^{-1} \ x", color=:green3)
